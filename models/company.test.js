...
describe("findAllFiltered", function () {
  ...
  test("works: filter finds no match", async function () {
    const filtered = {
      minEmployees: 4
    }
    let companies = await Company.findAllFiltered(filtered);
...
describe("_sqlForCompaniesFindAllFiltered", function () {
  test("works for all filters", function () {
    const { whereConds, values } = Company._sqlForCompaniesFindAllFiltered(
  ...
  test("fails min > max", function () {
    try {
      Company._sqlForCompaniesFindAllFiltered(
